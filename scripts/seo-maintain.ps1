...
<#
.SYNOPSIS
  SEO maintenance — internal link, local asset, and sitemap parity checks (888betlive static site).
...
.DESCRIPTION
  ...
  Absolute URLs are skipped unless they point at hosts we serve as this static site:
  pgasiagames.com (legacy sample) or 888betlive.com (production canonical host).
...
function Resolve-SiteUrlToPath {
    ...
    # SEO / parity: normalize production (888betlive) and legacy staging hostnames to filesystem paths.
    $p = $UrlPath `
        -replace '^https://888betlive\.com', '' `
        -replace '^http://888betlive\.com', '' `
        -replace '^https://pgasiagames\.com', '' `
        -replace '^http://pgasiagames\.com', '' `
        -replace '#.*$', '' `
        -replace '\?.*$', ''
    if ($p -eq "" -or $p -eq "/") {
...
function Resolve-HrefFromFile {
    ...
    if ($Href -match '^(https?:)?//' -and $Href -notmatch '(888betlive\.com|pgasiagames\.com)') {
...
foreach ($file in $htmlFiles) {
    ...
    foreach ($m in [regex]::Matches($text, $attrPattern)) {
        ...
        if ($val -match '^https?://(888betlive\.com|pgasiagames\.com)') {
